...
# Backup existing vsftpd configuration
echo "Backing up existing vsftpd configuration..."
sudo cp /etc/vsftpd.conf /etc/vsftpd.conf.bak || error_exit "Failed to backup vsftpd configuration."
...
if id "ftpuser" &>/dev/null; then
    ...
    read -sp "Enter password for ftpuser: " password
    echo "ftpuser:$password" | sudo chpasswd || error_exit "Failed to set password for ftpuser."
fi
